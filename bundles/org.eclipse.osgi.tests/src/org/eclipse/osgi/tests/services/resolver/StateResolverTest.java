...
public class StateResolverTest extends AbstractStateTest {
	...
	/**
	 * Tests adding 3 new bundles to an already resolved state and then resolving
	 * only one of the bundles. The result should be all 3 added bundles being
	 * resolved.
	 */
	@Test
	public void testAdd3Resolve1() throws BundleException {
		...
		assertEquals("2.1", (BundleDelta.RESOLVED | BundleDelta.ADDED), //$NON-NLS-1$
				((BundleDelta) deltasMap.get(Long.valueOf(1))).getType());
		assertEquals("2.2", (BundleDelta.RESOLVED | BundleDelta.ADDED), //$NON-NLS-1$
				((BundleDelta) deltasMap.get(Long.valueOf(2))).getType());
		assertEquals("2.3", (BundleDelta.RESOLVED | BundleDelta.ADDED), //$NON-NLS-1$
				((BundleDelta) deltasMap.get(Long.valueOf(3))).getType());
	}
	...
	public void testBasicResolution() throws BundleException {
		...
		// TODO why do we expect unresolved deltas here when the bundle was not resolved
		// in the first place?
		// BundleDelta[] unresolutions = delta.getChanges(BundleDelta.UNRESOLVED,
		// false);
	...
	public void testLinkageChange() throws BundleException {
		...
		BundleDescription b1 = state.getFactory().createBundleDescription(parseManifest(B1_MANIFEST), B1_LOCATION,
		...
		final String B2_MANIFEST = "Bundle-SymbolicName: org.eclipse.b2\n" + "Bundle-Version: 2.0\n" //$NON-NLS-1$ //$NON-NLS-2$
				+ "Require-Bundle: org.eclipse.b1;optional=true"; //$NON-NLS-1$
		BundleDescription b2 = state.getFactory().createBundleDescription(parseManifest(B2_MANIFEST), B2_LOCATION,
				(long) (Math.random() * Long.MAX_VALUE));
		...
		final String B3_MANIFEST = "Bundle-SymbolicName: org.eclipse.b3\n" + "Bundle-Version: 2.0\n" //$NON-NLS-1$ //$NON-NLS-2$
				+ "Require-Bundle: org.eclipse.b1"; //$NON-NLS-1$
		BundleDescription b3 = state.getFactory().createBundleDescription(parseManifest(B3_MANIFEST), B3_LOCATION,
				(long) (Math.random() * Long.MAX_VALUE));
		// add b2 and b3
		state.addBundle(b2);
		state.addBundle(b3);
		...
		// BundleDelta[] optionalLinkageChanged =
		// delta.getChanges(BundleDelta.OPTIONAL_LINKAGE_CHANGED, true);
	...
	public void testReinstall() throws BundleException {
		...
		delta = state.resolve();
		assertEquals("1.0", 1, delta //$NON-NLS-1$
				.getChanges(BundleDelta.REMOVED | BundleDelta.UNRESOLVED | BundleDelta.REMOVAL_COMPLETE, true).length);
		assertEquals("1.1", 4L, //$NON-NLS-1$
				delta.getChanges(BundleDelta.REMOVED | BundleDelta.UNRESOLVED | BundleDelta.REMOVAL_COMPLETE, true)[0]
		...
		BundleDescription b4 = delta
				.getChanges(BundleDelta.REMOVED | BundleDelta.UNRESOLVED | BundleDelta.REMOVAL_COMPLETE, true)[0]
				.getBundle();
		state.addBundle(b4);
		delta = state.resolve();
		assertEquals("3.0", 1, delta.getChanges(BundleDelta.ADDED | BundleDelta.RESOLVED, true).length); //$NON-NLS-1$
		assertEquals("3.1", 4L, //$NON-NLS-1$
				delta.getChanges(BundleDelta.ADDED | BundleDelta.RESOLVED, true)[0].getBundle().getBundleId());
	...
	public void testRemoval() throws BundleException {
		...
		assertEquals("1.4 - " + changes[0].getBundle(), (BundleDelta.ADDED | BundleDelta.RESOLVED), //$NON-NLS-1$
				changes[0].getType());
		assertEquals("1.5 - " + changes[1].getBundle(), (BundleDelta.ADDED | BundleDelta.RESOLVED), //$NON-NLS-1$
				changes[1].getType());
	 ...
	 * Tests adding 3 new bundles to an already resolved state and then resolving
	 * only one of the bundles. The result should be all 3 added bundles being
	 * resolved. Then re-resolving the same bundle. The result should be only the
	 * one bundle being resolved.
	 */
	@Test
	...
	public void testUpdate() throws BundleException {
		...
		BundleDescription b11 = state.getFactory().createBundleDescription(parseManifest(B1_UNRESOLVED), B1_LOCATION,
				1);
	...
	public void testSingletons() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		BundleDescription testDependent = state.getFactory().createBundleDescription(state, manifest, "test.frag101", //$NON-NLS-1$
				4);
	...
	public void testSingletonsSelection1() throws BundleException {
		...
		// test the selection algorithm of the resolver to pick the bundles with the
		// largest version
		Hashtable manifest = new Hashtable();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
	...
	public void testSingletonsSelection2() throws BundleException {
		...
		// test the selection algorithm of the resolver to pick the bundles with the
		// largest version; test with cycle added
		Hashtable manifest = new Hashtable();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "sdk; " + Constants.SINGLETON_DIRECTIVE + ":=true"); //$NON-NLS-1$ //$NON-NLS-2$
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
	...
	public void testSingletonsSelection3() throws BundleException {
		...
		int bundleID = 0;
		// test the selection algorithm of the resolver to pick the bundles with the
		// largest version; with fragments
		Hashtable manifest = new Hashtable();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "sdk; " + Constants.SINGLETON_DIRECTIVE + ":=true"); //$NON-NLS-1$ //$NON-NLS-2$
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		...
		BundleDescription sdk_frag10 = state.getFactory().createBundleDescription(state, manifest, "sdk.frag10", //$NON-NLS-1$
				bundleID++);
		...
		BundleDescription sdk_frag210 = state.getFactory().createBundleDescription(state, manifest, "sdk.frag210", //$NON-NLS-1$
				bundleID++);
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		manifest.put(Constants.REQUIRE_BUNDLE, "rcp; bundle-version=\"[1.0,2.0]\""); //$NON-NLS-1$
		BundleDescription platform10 = state.getFactory().createBundleDescription(state, manifest, "platform10", //$NON-NLS-1$
				bundleID++);
		...
		BundleDescription platform_frag10 = state.getFactory().createBundleDescription(state, manifest,
				"platform.frag10", bundleID++); //$NON-NLS-1$
		...
		BundleDescription platform_frag210 = state.getFactory().createBundleDescription(state, manifest,
				"platform.frag210", bundleID++); //$NON-NLS-1$
		...
		BundleDescription rcp_frag10 = state.getFactory().createBundleDescription(state, manifest, "rcp.frag10", //$NON-NLS-1$
				bundleID++);
		...
		BundleDescription rcp_frag210 = state.getFactory().createBundleDescription(state, manifest, "rcp.frag210", //$NON-NLS-1$
				bundleID++);
		...
		manifest.put(Constants.BUNDLE_VERSION, "2.0"); //$NON-NLS-1$
		manifest.put(Constants.REQUIRE_BUNDLE, "rcp; bundle-version=\"[1.0,2.0]\""); //$NON-NLS-1$
		BundleDescription platform20 = state.getFactory().createBundleDescription(state, manifest, "platform20", //$NON-NLS-1$
				bundleID++);
	...
	public void testSingletonsSelection4() throws BundleException {
		...
		int bundleID = 0;
		// test the selection algorithm of the resolver to pick the bundles with the
		// largest version; with fragments using Import-Package
		Hashtable manifest = new Hashtable();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "sdk; " + Constants.SINGLETON_DIRECTIVE + ":=true"); //$NON-NLS-1$ //$NON-NLS-2$
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
	...
	public void testSingletonsSelection6() throws BundleException {
		...
		// reorder the bundles to test that order of bundles does not effect resolution
		// outcome
		state.addBundle(sdk20);
		state.addBundle(platform20);
		state.addBundle(rcp20);
	...
	public void testRequireBundleUses() throws BundleException {
		...
		int index = c1ResolvedImports[0].getName().equals("a") ? 0 //$NON-NLS-1$
				: c1ResolvedImports[1].getName().equals("a") ? 1 : -1; //$NON-NLS-1$
	...
	public void testFragmentUses01() throws BundleException {
		...
		BundleDescription a1 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION), id++); // $NON-NLS-1$
		...
		BundleDescription a1frag = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION), id++); // $NON-NLS-1$
		...
		BundleDescription b1 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION), id++); // $NON-NLS-1$
	...
	public void testFragmentConstraints01() throws BundleException {
		...
		a1frag_100 = state.getFactory().createBundleDescription(state, manifest, "a1frag_100", //$NON-NLS-1$
				a1frag_100.getBundleId());
	...
	public void testFragmentConstraints02() throws BundleException {
		...
		BundleDescription a = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + "_" + manifest.get(Constants.BUNDLE_VERSION), //$NON-NLS-1$
				id++);
		...
		BundleDescription aFrag1 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + "_" + manifest.get(Constants.BUNDLE_VERSION), //$NON-NLS-1$
				id++);
		...
		BundleDescription aFrag2 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + "_" + manifest.get(Constants.BUNDLE_VERSION), //$NON-NLS-1$
				id++);
		...
		BundleDescription aFrag3 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + "_" + manifest.get(Constants.BUNDLE_VERSION), //$NON-NLS-1$
				id++);

		// Bug 353103 need to create a bundle that has the same unresolved imports as
		// other bundles
		manifest = new Hashtable();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		...
		BundleDescription aFrag4 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + "_" + manifest.get(Constants.BUNDLE_VERSION), //$NON-NLS-1$
				id++);
	...
	public void testPlatformProperties01() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		BundleDescription systemBundle = state.getFactory().createBundleDescription(state, manifest, "org.eclipse.osgi", //$NON-NLS-1$
				bundleID++);
		...
		props[1].put("org.osgi.framework.system.capabilities",
		...
		// now test the uses clause for pkg.b such that bundle 'A' will be forced to
		// used
		...
		// now test the uses clause for pkg.d such that bundle 'C' will be forced to
		// used
		// pkg.system.b from bundle 'system.b'
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "D"); //$NON-NLS-1$
	...
	public void testPlatformProperties02() throws BundleException {
		// same as 01 except use alias system.bundle to another name
		// "test.system.bundle"
		State state = buildEmptyState();
		int bundleID = 0;
		// test the selection algorithm of the resolver to pick the bundles which
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		BundleDescription systemBundle = state.getFactory().createBundleDescription(state, manifest,
				"test.system.bundle", bundleID++); //$NON-NLS-1$
		...
		props[0].put("org.osgi.framework.system.packages", "pkg.system.a, pkg.system.c"); //$NON-NLS-1$ //$NON-NLS-2$
		props[0].put("org.osgi.framework.executionenvironment", "J2SE-1.2"); //$NON-NLS-1$ //$NON-NLS-2$
		props[0].put("osgi.system.bundle", "test.system.bundle"); // set the system.bundle //$NON-NLS-1$ //$NON-NLS-2$
																	// to another system bundle (other than
																	// org.eclipse.osgi)
		props[1].put("org.osgi.framework.system.packages", "pkg.system.a, pkg.system.b, pkg.system.c"); //$NON-NLS-1$ //$NON-NLS-2$
		props[1].put("org.osgi.framework.executionenvironment", "J2SE-1.4"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void testPlatformProperties03() throws BundleException {
		// test that require-bundle, fragment-host, and import-package of system.bundle
		// can be aliased properly
		State state = buildEmptyState();
		int bundleID = 0;
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		BundleDescription systemBundle = state.getFactory().createBundleDescription(state, manifest,
				configuredSystemBundle, bundleID++);
		...
		manifest.put(Constants.IMPORT_PACKAGE,
				"system.bundle.exported.pkg; bundle-symbolic-name=\"" + configuredSystemBundle + "\""); //$NON-NLS-1$ //$NON-NLS-2$
		BundleDescription d = state.getFactory().createBundleDescription(state, manifest, "D", bundleID++); //$NON-NLS-1$
	...
	public void testPlatformProperties04() throws BundleException {
		// same as 03 except use a different system.bundle alias other than
		// org.eclipse.osgi
		// test that require-bundle, fragment-host, and import-package of system.bundle
		// can be aliased properly
		State state = buildEmptyState();
		...
		Dictionary[] props = new Dictionary[] { new Hashtable(), new Hashtable() };
		props[0].put("org.osgi.framework.system.packages", "pkg.system.a, pkg.system.b"); //$NON-NLS-1$ //$NON-NLS-2$
		props[0].put("osgi.system.bundle", configuredSystemBundle); // set the system.bundle to another //$NON-NLS-1$
																	// system bundle (other than org.eclipse.osgi)
	...
	public void testPlatformPropertiesBug207500b() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"libwrapper-linux-x86-32.so; wrapper-linux-x86-32; osname=linux; processor=x86"); //$NON-NLS-1$
		BundleDescription a = state.getFactory().createBundleDescription(state, manifest, "A", bundleID++); //$NON-NLS-1$
	...
	public void testPlatformPropertiesBug246640a() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, configuredSystemBundle);
		manifest.put(Constants.BUNDLE_VERSION, "1.0"); //$NON-NLS-1$
		BundleDescription systemBundle1 = state.getFactory().createBundleDescription(state, manifest,
				configuredSystemBundle + manifest.get(Constants.BUNDLE_VERSION), bundleID++);

		manifest.clear();
		manifest.put(Constants.BUNDLE_MANIFESTVERSION, "2"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, configuredSystemBundle);
		manifest.put(Constants.BUNDLE_VERSION, "2.0"); //$NON-NLS-1$
		BundleDescription systemBundle2 = state.getFactory().createBundleDescription(state, manifest,
				configuredSystemBundle + manifest.get(Constants.BUNDLE_VERSION), bundleID++);
		...
		manifest.put(Constants.BUNDLE_REQUIREDEXECUTIONENVIRONMENT, "J2SE-1.2"); //$NON-NLS-1$
		BundleDescription requireSystemBundle = state.getFactory().createBundleDescription(state, manifest,
	...
	public void testEECapabilityRequirement() throws BundleException {
		...
		Dictionary[] props = new Dictionary[] { new Hashtable(), new Hashtable(), new Hashtable(), new Hashtable(),
		...
		props[7].put("org.osgi.framework.executionenvironment", "II-1.0/JJ-2.0");
		props[0].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"OSGi/Minimum\"; version:List<Version>=\"1.0, 1.1, 1.2\""); //$NON-NLS-1$
		props[1].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"CDC/Foundation\"; version:List<Version>=\"1.0, 1.1\""); //$NON-NLS-1$
		props[2].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"JavaSE\"; version:List<Version>=\"1.2, 1.3, 1.4, 1.5, 1.6\""); //$NON-NLS-1$
		...
		props[5].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"EE/FF-YY\"; version:List<Version>=\"1.0, 2.0\""); //$NON-NLS-1$
		props[6].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"GG-XX/HH\"; version:List<Version>=\"1.0, 2.0\""); //$NON-NLS-1$
	...
	public void testEECapabilityRequirement1() throws BundleException {
		...
		props[0].put("org.osgi.framework.executionenvironment", //$NON-NLS-1$
				"OSGi/Minimum-1.0, OSGi/Minimum-1.1, OSGi/Minimum-1.2, CDC-1.0/Foundation-1.0, CDC-1.1/Foundation-1.1, AA-BB"); //$NON-NLS-1$
		props[1].put("org.osgi.framework.executionenvironment", //$NON-NLS-1$
				"OSGi/Minimum-1.0, OSGi/Minimum-1.1, OSGi/Minimum-1.2, JRE-1.1, J2SE-1.2, J2SE-1.3, J2SE-1.4"); //$NON-NLS-1$
		props[2].put("org.osgi.framework.executionenvironment", //$NON-NLS-1$
				"OSGi/Minimum-1.0, OSGi/Minimum-1.1, JRE-1.1, J2SE-1.2, J2SE-1.3"); //$NON-NLS-1$
		...
		props[0].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"OSGi/Minimum\"; version:List<Version>=\"1.0, 1.1, 1.2\", osgi.ee; osgi.ee=\"CDC/Foundation\"; version:List<Version>=\"1.0, 1.1\", osgi.ee; osgi.ee=\"AA-BB\""); //$NON-NLS-1$
		props[1].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"OSGi/Minimum\"; version:List<Version>=\"1.0, 1.1, 1.2\", osgi.ee; osgi.ee=\"JavaSE\"; version:List<Version>=\"1.0, 1.1, 1.2, 1.3, 1.4\""); //$NON-NLS-1$
		props[2].put("org.osgi.framework.system.capabilities", //$NON-NLS-1$
				"osgi.ee; osgi.ee=\"OSGi/Minimum\"; version:List<Version>=\"1.0, 1.1\", osgi.ee; osgi.ee=\"JavaSE\"; version:List<Version>=\"1.0, 1.1, 1.2, 1.3\""); //$NON-NLS-1$
	...
	public void testImportJavaPackages() throws Exception {
		...
		BundleDescription systemBundle = state.getFactory().createBundleDescription(state, manifest, "org.eclipse.osgi",
				bundleID++);
		state.addBundle(systemBundle);
	...
	private ExportPackageDescription[] isConsistent(ExportPackageDescription[] pkgs1,
			ExportPackageDescription[] pkgs2) {
	...
	public void testSelectionPolicy() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "test.host; singleton:=true"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		BundleDescription testHost100 = state.getFactory().createBundleDescription(state, manifest, "test.host100", //$NON-NLS-1$
				bundleID++);
		...
		manifest.put(Constants.BUNDLE_SYMBOLICNAME, "test.host; singleton:=true"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_VERSION, "1.0.1"); //$NON-NLS-1$
		BundleDescription testHost101 = state.getFactory().createBundleDescription(state, manifest, "test.host101", //$NON-NLS-1$
				bundleID++);
		...
		manifest.put(Constants.FRAGMENT_HOST, "test.host; bundle-version=\"[1.0.0,2.0.0)\""); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		BundleDescription testFrag100 = state.getFactory().createBundleDescription(state, manifest, "test.frag100", //$NON-NLS-1$
				bundleID++);
		...
		manifest.put(Constants.FRAGMENT_HOST, "test.host; bundle-version=\"[1.0.0,2.0.0)\""); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_VERSION, "1.0.1"); //$NON-NLS-1$
		BundleDescription testFrag101 = state.getFactory().createBundleDescription(state, manifest, "test.frag101", //$NON-NLS-1$
				bundleID++);
	...
	public void testNativeCodeResolution01() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=x86;osname=Windows2000;osname=\"Windows 2003\";osname=Windows95;osname=Windows98;osname=WindowsNT;osname=WindowsXP;osname=\"Windows NT (unknown)\";osname=\"Windows Vista\"; language=en"); //$NON-NLS-1$
		BundleDescription testNativeBundle = state.getFactory().createBundleDescription(state, manifest, "NativeBundle", //$NON-NLS-1$
				bundleID++);
	...
	public void testNativeCodeResolution02() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=x86;osname=Windows2000;osname=\"Windows 2003\";osname=Windows95;osname=Windows98;osname=WindowsNT;osname=WindowsXP;osname=\"Windows NT (unknown)\";osname=\"Windows Vista\""); //$NON-NLS-1$
		BundleDescription testNativeBundle = state.getFactory().createBundleDescription(state, manifest, "NativeBundle", //$NON-NLS-1$
				bundleID++);
		state.addBundle(testNativeBundle);
		state.resolve();
	...
	public void testNativeCodeResolution04() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=iNvAlid;osname=doeSnoTexist"); //$NON-NLS-1$
		BundleDescription testNativeBundle = state.getFactory().createBundleDescription(state, manifest, "NativeBundle", //$NON-NLS-1$
				bundleID++);
		state.addBundle(testNativeBundle);
		state.resolve();
	...
	public void testNativeCodeResolution05() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=x86;osname=win32; language=en"); //$NON-NLS-1$
		BundleDescription testNativeBundle1 = state.getFactory().createBundleDescription(state, manifest,
				"NativeBundle", bundleID++); //$NON-NLS-1$
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=x86_64;osname=win32; language=en"); //$NON-NLS-1$
		BundleDescription testNativeBundle2 = state.getFactory().createBundleDescription(state, manifest,
				"NativeBundle", bundleID++); //$NON-NLS-1$
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.BUNDLE_NATIVECODE,
				"Bundle-NativeCode: nativefile1.txt;processor=x86;osname=win32; language=fr"); //$NON-NLS-1$
		BundleDescription testNativeBundle3 = state.getFactory().createBundleDescription(state, manifest,
				"NativeBundle", bundleID++); //$NON-NLS-1$
	...
	private State createBug266935State() throws BundleException {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		BundleDescription a = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
	...
	private BundleDescription updateStateBug266935(State state, BundleDescription a) throws BundleException {
		...
		BundleDescription newA = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				a.getBundleId());
	...
	public void testBug320124() throws BundleException {
		...
		ExportPackageDescription[] visible = state.getStateHelper().getVisiblePackages(a,
	...
	public void testRequirements() throws BundleException, InvalidSyntaxException, IOException {
		...
		BundleDescription hostDescription = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME), bundleID++);
		state.addBundle(hostDescription);
		manifest = loadManifest("r2.MF");
		BundleDescription fragDescription = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME), bundleID++);
	...
	private void doTestRequirements(BundleRevision hostRevision, BundleRevision fragRevision)
		...
		checkRequirement((BundleRequirement) pReqs.get(7), matchingAttrs, null,
				"(&(resolution=optional)(d1=v1)(d2=v2))", true);
		...
		checkRequirement((BundleRequirement) bReqs.get(0), matchingAttrs, null,
				"(&(visibility=reexport)(resolution=optional))", true);

		matchingAttrs.put(BundleRevision.BUNDLE_NAMESPACE, "requirer.b2");
		checkRequirement((BundleRequirement) bReqs.get(1), matchingAttrs, null,
				"(&(|(visibility=private)(!(visibility=*)))(|(resolution=mandatory)(!(resolution=*))))", true);
		...
		matchingAttrs.put("a1", "v1");
		matchingAttrs.put("a2", "v2");
		checkRequirement((BundleRequirement) bReqs.get(7), matchingAttrs, null,
				"(&(resolution=optional)(d1=v1)(d2=v2))", true);
		...
		matchingAttrs.put("a1", "v1");
		checkRequirement((BundleRequirement) cReqs.get(0), matchingAttrs, null,
				"(&(|(effective=resolve)(!(effective=*)))(resolution=optional)(filter=\\(a1=v1\\)))", true);

		cReqs = hostRevision.getDeclaredRequirements("require.c2");
		assertEquals("Wrong number of reqs: require.c2", 1, cReqs.size());
		matchingAttrs.clear();
		checkRequirement((BundleRequirement) cReqs.get(0), matchingAttrs, null,
				"(&(|(effective=resolve)(!(effective=*)))(|(resolution=mandatory)(!(resolution=*))))", false);

		cReqs = hostRevision.getDeclaredRequirements("require.c3");
		assertEquals("Wrong number of reqs: require.c3", 1, cReqs.size());
		matchingAttrs.clear();
		checkRequirement((BundleRequirement) cReqs.get(0), matchingAttrs, "(&(a1=v1)(a2=v2))",
				"(&(resolution=optional)(d1=v1)(d2=v2))", false);
	...
	private void checkRequirement(BundleRequirement req, Map matchingAttrs, String attributesFilterSpec,
			String directivesFilterSpec, boolean reqFilter) throws InvalidSyntaxException {
		...
		if (directivesFilterSpec != null) {
			...
			assertTrue("Cannot match directive filter: " + directivesFilterSpec,
					directivesFilter.matches(req.getDirectives()));
		}
	}
	...
	private void doTestCapabilities(BundleRevision hostRevision) throws InvalidSyntaxException {
		...
		checkCapability((BundleCapability) bundleCaps.get(0), "(&(osgi.wiring.bundle=r1)(a1=v1)(a2=v2))",
				"(&(d1=v1)(d2=v2)(singleton=true)(fragment-attachment=resolve-time))");
		...
		checkCapability((BundleCapability) hostCaps.get(0), "(&(osgi.wiring.host=r1)(a1=v1)(a2=v2))",
				"(&(d1=v1)(d2=v2)(singleton=true)(fragment-attachment=resolve-time))");
		...
		checkCapability((BundleCapability) pkgCaps.get(0),
				"(&(osgi.wiring.package=exporter.pkg1)(bundle-symbolic-name=r1)(bundle-version=1.0)(version=1.0))",
				null);
		checkCapability((BundleCapability) pkgCaps.get(1),
				"(&(osgi.wiring.package=exporter.pkg2)(bundle-symbolic-name=r1)(bundle-version=1.0)(a1=v1)(a2=v2))",
				"(&(include=C1,C2)(exclude=C3,C4)(mandatory=a1,a2)(uses=importer.pkg1,importer.pkg2))");
		checkCapability((BundleCapability) pkgCaps.get(2),
				"(&(osgi.wiring.package=exporter.pkg3)(bundle-symbolic-name=r1)(bundle-version=1.0)(a1=v1)(a2=v2))",
				"(&(mandatory=a1,a2)(d1=v1)(d2=v2))");

		List gCaps = hostRevision.getDeclaredCapabilities("provide.c1");
		assertEquals("Wrong number of: provide.c1", 1, gCaps.size());
		checkCapability((BundleCapability) gCaps.get(0), "(&(a1=v1)(a2=v2))",
				"(&(|(effective=resolve)(!(effective=*)))(uses=importer.pkg1,importer.pkg2))");

		gCaps = hostRevision.getDeclaredCapabilities("provide.c2");
		assertEquals("Wrong number of: provide.c1", 1, gCaps.size());
		checkCapability((BundleCapability) gCaps.get(0), "(&(a1=v1)(a2=v2))",
				"(&(|(effective=resolve)(!(effective=*)))(uses=importer.pkg1,importer.pkg2)(d1=v1)(d2=v2))");

	}

	private void checkCapability(BundleCapability cap, String attributeFilterSpec, String directiveFilterSpec)
			throws InvalidSyntaxException {
		...
		if (directiveFilterSpec != null) {
			...
			assertTrue("Cannot match directive filter: " + directivesFilter,
					directivesFilter.matches(cap.getDirectives()));
		}
	}
	...
	public void testResolveFragmentEE01() throws BundleException {
		...
		b = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				b.getBundleId());
	...
	/**
	 * Test for provoking a flaw in VersionHashMap.compare(...) when sorting the
	 * VersionSupplier: Comparison method violates its general contracts. Not easy
	 * to preproduce, as it depends heavily on the internal sort algorithm, TimSort,
	...
	public void testIssue156() throws Exception {
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.PROVIDE_CAPABILITY,
				"osgi.service;objectClass=\"dnvsidDPUS,ovbsISISDP\",osgi.service;objectClass=\"dnvsidHDPUS,ovbsISISDP\",osgi.service;objectClass=\"dnggaIM,dnggarGRF\",osgi.service;objectClass=\"dnvecaCFSS\"");
		BundleDescription b2 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b2);
		...
		BundleDescription b3 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b3);
		...
		BundleDescription b4 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b4);
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.PROVIDE_CAPABILITY,
				"osgi.service;objectClass=\"dngpaXGAIDG\",osgi.service;objectClass=\"ovbcsIAS,ovbcICL\",osgi.service;objectClass=\"ovbppIRHF\",osgi.service;objectClass=\"dngpraRF\",osgi.service;objectClass=\"ovbrIRL\",osgi.service;objectClass=\"jsS,ovbphRSS\",osgi.service;objectClass=\"ovbcIM2PCF\",osgi.service;objectClass=\"ovbcsITS\"");
		BundleDescription b5 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b5);
		...
		BundleDescription b6 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b6);
		...
		BundleDescription b7 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b7);
		...
		manifest.put(Constants.BUNDLE_VERSION, "1.0.0"); //$NON-NLS-1$
		manifest.put(Constants.PROVIDE_CAPABILITY,
				"osgi.service;objectClass=\"jsS\",osgi.service;objectClass=\"ovbcICL,ovbea.IEN\",osgi.service;objectClass=\"ovbeaIEF,ovbrIRM\",osgi.service;objectClass=\"ovbeaIIP\",osgi.service;objectClass=\"ovbeiECDS\"");
		BundleDescription b8 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b8);
		...
		BundleDescription b9 = state.getFactory().createBundleDescription(state, manifest,
				(String) manifest.get(Constants.BUNDLE_SYMBOLICNAME) + manifest.get(Constants.BUNDLE_VERSION),
				bundleID++);
		state.addBundle(b9);

		// throws an IllegalArgumentException("Comparison method violates its general
		// contract!")
		state.resolve(false);
	}
