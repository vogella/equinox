...
public interface State {
	 ...
	 * <p>
	 * If the bundle already exists in another state then an
	 * <code>IllegalStateException</code> will be thrown. Note that even if you
	 * remove a <code>BundleDescription</code> from one <code>State</code> object
	 * using {@link State#removeBundle(BundleDescription)} it may still be
	 * considered as removing pending if other bundles in that state depend on the
	...
	/**
	 * Returns a delta describing the differences between this state and the given
	 * state. The given state is taken as the base so the absence of a bundle in
	 * this state is reported as a deletion, etc.
	 * <p>
	 * Note that the generated StateDelta will contain BundleDeltas with one of the
	 * following types: BundleDelta.ADDED, BundleDelta.REMOVED and
	 * BundleDelta.UPDATED
	 * </p>
	 *
	...
	/**
	 * Returns the delta representing the changes from the time this state was first
	 * captured until now.
	 *
	 ...
	 * <code>null</code> is returned if no such bundle is found in this state.
	 *
	 ...
	 * A null value is returned if no such bundle is found in this state. A resolved
	 * bundle is always preferably returned over an unresolved bundle. If multiple
	 * bundles with the same resolution state are available, the bundle with the
	 * highest version number is returned if the <code>version</code> is null.
	 *
	...
	/**
	 * Returns the timestamp for this state. This correlates this timestamp to the
	 * system state. For example, if the system state timestamp is 4 but then some
	 * bundles are installed, the system state timestamp is updated. By comparing 4
	 * to the current system state timestamp it is possible to detect if the states
	 * are out of sync.
	 *
	...
	/**
	 * Returns true if there have been no modifications to this state since the last
	 ...
	 * state. Note that a constraint can be unresolved by passing null for the
	 * supplier.
	 * <p>
	 * This method is intended to be used by resolvers in the process of determining
	 * which constraints are satisfied by which components.
	 * </p>
	 *
	 ...
	 * @param status           whether or not the given bundle is resolved, if false
	 *                         the other parameters are ignored
	 * @param hosts            the host for the resolve fragment, can be
	 *                         <code>null</code>
	 * @param selectedExports  the selected exported packages for this resolved
	 *                         bundle, can be <code>null</code>
	 * @param resolvedRequires the {@link BundleDescription}s that resolve the
	 *                         required bundles for this bundle, can be
	 *                         <code>null</code>
	 * @param resolvedImports  the exported packages that resolve the imports for
	 *                         this bundle, can be <code>null</code>
	 * @throws IllegalStateException if this is not done during a call to
	 *                               <code>resolve</code>
	 * @deprecated use
	 ...
	 *
	 * @param bundle             the bundle to update
	 * @param status             whether or not the given bundle is resolved, if
	 *                           false the other parameters are ignored
	 * @param hosts              the host for the resolve fragment, can be
	 *                           <code>null</code>
	 * @param selectedExports    the selected exported packages for this resolved
	 *                           bundle, can be <code>null</code>
	 * @param substitutedExports the exported packages that resolve imports for this
	 *                           bundle and substitute exports, can be
	 *                           <code>null</code>
	 * @param resolvedRequires   the {@link BundleDescription}s that resolve the
	 *                           required bundles for this bundle, can be
	 *                           <code>null</code>
	 ...
	 * @param selectedExports      the selected exported packages for this resolved
	 *                             bundle, can be <code>null</code>
	 * @param substitutedExports   the exported packages that resolve imports for
	 *                             this bundle and substitute exports, can be
	 *                             <code>null</code>
	 * @param selectedCapabilities the selected capabilities for this resolved
	 *                             bundle, can be <code>null</code>
	 * @param resolvedRequires     the {@link BundleDescription}s that resolve the
	 *                             required bundles for this bundle, can be
	 *                             <code>null</code>
	 * @param resolvedImports      the exported packages that resolve the imports
	 *                             for this bundle, can be <code>null</code>
	 * @param resolvedCapabilities the capabilities that resolve the required
	 *                             capabilities for this bundle, can be
	 *                             <code>null</code>
	 * @param resolvedWires        the map of state wires for the resolved
	 *                             requirements of the given bundle. The key is the
	 *                             name space of the requirement.
	 * @throws IllegalStateException if this is not done during a call to
	 *                               <code>resolve</code>
	 * @since 3.7
	 */
	public void resolveBundle(BundleDescription bundle, boolean status, BundleDescription[] hosts,
			ExportPackageDescription[] selectedExports, ExportPackageDescription[] substitutedExports,
			GenericDescription[] selectedCapabilities, BundleDescription[] resolvedRequires,
			ExportPackageDescription[] resolvedImports, GenericDescription[] resolvedCapabilities,
	 ...
	 * @param unsatisfied the unsatisfied constraint or null if the resolver error
	 *                    was not caused by an unsatisfied constraint.
	 * @throws IllegalStateException if this is not done during a call to
	 *                               <code>resolve</code>
	 * @since 3.2
	 */
	...
	/**
	 * Returns the resolver associated with this state. A state can work with at
	 * most one resolver at any given time. Similarly, a resolver can work with at
	 * most one state at a time.
	 *
	 * @return the resolver for this state. null is returned if the state does not
	 *         have a resolver
	 */
	public Resolver getResolver();

	/**
	 * Sets the resolver associated with this state. A state can work with at most
	 * one resolver at any given time. Similarly, a resolver can work with at most
	 * one state at a time.
	 * <p>
	 * To ensure that this state and the given resovler are properly linked, the
	 * following expression must be included in this method if the given resolver
	 * (value) is not identical to the result of this.getResolver().
	 * </p>
	 *
	 * <pre>
	...
	/**
	 * Resolves the constraints contained in this state using the resolver currently
	 * associated with the state and returns a delta describing the changes in
	 * resolved states and dependencies in the state.
	 * <p>
	 ...
	 * should refuse to perform updates (@see #select() and #resolveConstraint()) if
	 * they are not currently involved in a resolution cycle.
	 * <p>
	 * Note the given state is destructively modified to reflect the results of
	 * resolution.
	 * </p>
	 *
	 * @param incremental a flag controlling whether resolution should be
	 *                    incremental
	 * @return a delta describing the changes in resolved state and interconnections
	 */
	...
	/**
	 * Resolves the constraints contained in this state using the resolver currently
	 * associated with the state in an incremental, "least-perturbing" mode, and
	 * returns a delta describing the changes in resolved states and dependencies in
	 * the state.
	 *
	 * @param discard an array containing descriptions for bundles whose current
	 *                resolution state should be forgotten. If <code>null</code>
	 *                then all the current removal pending BundleDescriptions are
	 *                refreshed.
	 * @return a delta describing the changes in resolved state and interconnections
	 */
	 ...
	 * associated with the state in an incremental, "least-perturbing" mode, and
	 * returns a delta describing the changes in resolved states and dependencies in
	 * the state. If discard is set to true the the descriptions contained in the
	 * resolve array will have their current resolution state discarded and will be
	 * re-resolved. This method will attempt to resolve the supplied descriptions
	 * and may attempt to resolve any other unresolved descriptions contained in
	 * this state.
	 *
	 * @param resolve an array containing descriptions for bundles to resolve.
	 * @param discard a value of true indicates the resolve descriptions should have
	 *                their current resolution state discarded and re-resolved.
	...
	/**
	 * Sets the version overrides which are to be applied during the resolution of
	 * this state. Version overrides allow external forces to refine/override the
	 * version constraints setup by the components in the state.
	 *
	 ...
	 *
	 * @return the descriptions for all bundles currently resolved in this state.
	 */
	 ...
	 * currently exported by a bundle in the graph or requires a bundle in the
	 * graph. The graph is fully constructed when there is no bundle outside the
	 * graph that is wired to a bundle in the graph. The graph may contain removal
	 * pending bundles.
	 *
	 ...
	 *         specified bundles, or an empty collection if there were no specified
	 *         bundles.
	 * @since 3.7
	 */
	...
	/**
	 * Sets the platform properties of the state. The platform properties are used
	 * to resolve the following constraints:
	 * <ul>
	 * <li>The execution environment requirements (i.e.
	 * Bundle-RequiredExecutionEnvironment).</li>
	 ...
	 * </ul>
	 * Arbitrary keys may be used in the platform properties but the following keys
	 * have a specified meaning:
	 * <ul>
	 ...
	 * <li>osgi.resolverMode - the resolver mode. A value of "strict" will set the
	 * resolver mode to strict.</li>
	 * <li>org.osgi.framework.system.packages - the packages exported by the system
	 * bundle.</li>
	 * <li>org.osgi.framework.executionenvironment - the comma separated list of
	 * supported execution environments. This property is then used to resolve the
	 * required execution environment the bundles in a state.</li>
	 * <li>org.osgi.framework.os.name - the name of the operating system. This
	 * property is used to resolve the osname attribute of bundle native code (i.e.
	 * Bundle-NativeCode).</li>
	 * <li>org.osgi.framework.os.version - the version of the operating system. This
	 * property is used to resolve the osversion attribute of bundle native code
	 * (i.e. Bundle-NativeCode).</li>
	 * <li>org.osgi.framework.processor - the processor name. This property is used
	 * to resolve the processor attribute of bundle native code (i.e.
	 * Bundle-NativeCode).</li>
	 * <li>org.osgi.framework.language - the language being used. This property is
	 * used to resolve the language attribute of bundle native code (i.e.
	 * Bundle-NativeCode).</li>
	 * </ul>
	 ...
	 *         values are changed as a result of calling this method then true is
	 *         returned.
	 */
	...
	/**
	 * Returns a state helper object. State helpers provide convenience methods for
	 * manipulating states.
	 * <p>
	 * A possible implementation for this method would provide the same single
	 * StateHelper instance to all clients.
	 * </p>
	 *
	...
	/**
	 * Returns the highest bundle ID. The value -1 is returned if no bundles exist
	 * in this state.
	 * <p>
